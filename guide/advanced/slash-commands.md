...
- [Claude Code changelog (v2.1.152–v2.1.212)](https://code.claude.com/docs/en/changelog) — `disallowed-tools` frontmatter on slash commands and skills (v2.1.152); subagents can spawn subagents up to 5 levels deep (v2.1.172, 2026-06-09); subagents run in background by default and inherit session extended-thinking config (v2.1.198, 2026-07-01); `/fork` now copies the conversation into a background session while the in-session subagent launcher becomes `/subtask`, plus a per-session subagent spawn cap of 200 (`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`, reset by `/clear`) (v2.1.212, 2026-07-17); verified 2026-07-18 (this run).
