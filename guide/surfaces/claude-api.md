...
- Default model IDs change. The current generally-available line is **Sonnet 5** (`claude-sonnet-5`, the `sonnet` alias), **Opus 4.8** (`claude-opus-4-8`, the `opus` alias), and Haiku 4.5. Sonnet 5 launched 2026-06-30 with a 1M-token context window (default and max) and intro pricing of $2 / $10 per million input / output tokens through 2026-08-31 (then $3 / $15). Model IDs are dateless pinned snapshots from the 4.6 generation on. On Bedrock / Vertex / Foundry the aliases lag — pin the full model ID or set `ANTHROPIC_DEFAULT_OPUS_MODEL` to override.
