...
- **Background sessions** — `claude --bg` starts a long-running Claude Code task; reattach later via `/resume` or `claude agents`. Spans CLI and `claude.ai/code`. Sessions on different machines can now message each other directly — `SendMessage` sends, `ListAgents` discovers reachable sessions (macOS and Linux only, shipped 2026-08-07 in v2.1.224).
...
- [Claude Code changelog (v2.1.224, 2026-08-07)](https://code.claude.com/docs/en/changelog) — Anthropic docs; verified 2026-08-08 (this run) — `claude self-hosted-runner` (Team/Enterprise, runs Claude Code web/mobile/desktop sessions on your own machines/containers) and cross-session `SendMessage` / `ListAgents` (macOS and Linux only).
