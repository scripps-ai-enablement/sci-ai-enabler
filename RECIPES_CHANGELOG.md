...
### Blocked
- **Localize a fiber/probe implant tip in an Allen CCF subregion from 2D coronal histology** (Problem class: Data analysis / Workflow automation; Neuroscience) — no recipe shipped; blocked on a grounding gap and routed to the catalog chain. @goodb (#74) asked for a QUINT-replacement Claude Code workflow that batches over 2D coronal histology sections, reports which Allen CCF subregion each fiber/probe tip lands in, and gives a hit/miss verdict against the intended target. Immediate-mode re-check confirmed the two load-bearing components remain uncatalogued — **DeepSlice** (coronal cutting-plane prediction, the hard step) and **brainglobe-atlasapi** (CCF voxel→structure lookup, the subregion readout). The catalogued neighbours don't substitute: [allenbrain-mcp](catalog/tools/allenbrain.html) is read-only RMA/ontology/image queries (Alpha, no LICENSE) and the [SimpleITK skill](catalog/tools/simpleitk-image-registration.html) is general 3D registration with no Allen atlas or cutting-plane model — a naive affine warp is exactly the off-axis failure the requester flagged. Per the cookbook's hard grounding rule, moved the request to `## User requests (blocked)` (`blocked-on=catalog:brainglobe-atlasapi,deepslice`), keeping the `## Missing components` brief (brainglobe-atlasapi + DeepSlice; DeepSlice license unverified) and `## Deferred — next-run priority` rung-3 recipe note in `recipes/curator-state.md`. The issue stays open; the rung-3 recipe writes cleanly once those components land.
