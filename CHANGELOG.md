...
Batch ingest of two life-science skill collections, following the K-Dense precedent (2026-06-04). Generalised the K-Dense-only ingester into a reusable, collection-parameterised pipeline — `scripts/ingest_collection.py` driven by `scripts/collections.yaml` (per-collection registry) + an auditable `scripts/<collection>_category_map.yaml` (include = CREATE a new page, augment = add this collection's install path to an existing page, skip = recorded out-of-scope). It handles flat and nested `skills/` layouts and idempotent augmentation (a `<!-- alt-install:<key> -->` sentinel). Catalog tool pages: 179 → 337.

### Added
- **SciAgent-Skills** (`jaechang-hits/SciAgent-Skills`, 197 skills, CC BY 4.0, BixBench-evaluated) — **90 new pages**: genomics (BWA-MEM2, STAR, Salmon, SAMtools/BCFtools, GATK, CNVkit, SnpEff, PLINK2, DESeq2, featureCounts, GSEApy, fastp, MultiQC, BEDtools, CellTypist, Harmony, popV) and ~25 reference-database skills (Ensembl, ENCODE, ClinVar, dbSNP, gnomAD, COSMIC, cBioPortal, GWAS Catalog, ClinPGx, JASPAR, ReMap, RegulomeDB, QuickGO, Monarch, KEGG, Reactome, STRING, BRENDA, ARCHS4, GEO, NCBI Gene, UCSC, Mouse Phenome Database, **ZINC**, openFDA, DailyMed, DDInter, Open Targets, GtoPdb, UniChem, EMDB, HMDB, Metabolomics Workbench, PRIDE, InterPro, MaxQuant); docking & MD (AutoDock Vina, smina, MDAnalysis, MDTraj); bio-imaging (Cellpose, napari, OpenCV, PyImageJ, scikit-image, trackpy); plus SAR analysis, ViennaRNA, pLannotate, sgRNA design, HOMER, MACS3, CellChat, libSBML, MOFA+, muon, SpikeInterface, nnU-Net, SimpleITK, Snakemake, Plotly, OpenAlex, bioRxiv, USPTO. **92 existing pages** gained a SciAgent alternative install path (one entry per tool). 15 out of scope — recorded in `scripts/sciagent_category_map.yaml`.
...
### Updated
- 91 existing tool pages augmented with a SciAgent (90) or NeuroClaw (1) alternative install path. AGENT.md community-collections table + batch-ingest note updated; `scripts/ingest_collection.py` + `scripts/collections.yaml` added; both collections moved to diff-only mode. `curator-state.md`: ZINC, Ensembl/UCSC MCP, and mne-neurophysiology deferred entries updated to reflect now-catalogued coverage.
