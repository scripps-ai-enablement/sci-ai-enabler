...
### Verified (no changes)
- Manifest sweep: `anthropics/life-sciences` marketplace re-fetched — 20 plugins, all already catalogued, zero diff.
- Evaluated and not catalogued: `sensein/brainkb_mcp` (no LICENSE; remote endpoint documented as "future"), `scitex-ai/scitex-dataset` (already covered by SciTeX Dataset MCP), SpikeLab and allenbrain-mcp (already catalogued), Neuronpedia MCP (language-model interpretability, not neuroscience), CatalystNeuro Read Images (general vision server).

## 2026-08-15

### Added
- **UCSC Cell Browser MCP** (Categories: Molecular and Cellular Biology) — MCPmed's BSD-3-Clause server over the public `cells.ucsc.edu` dataset index; seven read-only tools for finding scRNA-seq datasets by organism, tissue, disease or project, no API key. The page corrects two upstream README errors: the advertised PyPI package does not exist (404 as of 2026-08-15) and the clone directory is `UCSCCBmcp`, so it ships a source install plus literal Claude Code and Claude Desktop registration snippets ([repo](https://github.com/MCPmed/UCSCCBmcp), [MCPmed, *Brief Bioinform* 2026;27(1):bbag076](https://academic.oup.com/bib/article/27/1/bbag076/8495038))
- **PBMCpedia MCP** (Categories: Immunology and Microbiology, Molecular and Cellular Biology) — MCPmed/Saarland CCB server over a harmonized PBMC single-cell atlas with TCR/BCR and CITE-seq layers; six tools for expression, DEGs, pathways, cohort metadata and antibody chains, filterable by sex, age group, disease and cell type. Tool list read from `server.ts` because the README documents none; the page calls out the `cwd` trap that otherwise stops the stdio server launching ([repo](https://github.com/MCPmed/PBMCpediaMCP), [*NAR* doi:10.1093/nar/gkaf1245](https://doi.org/10.1093/nar/gkaf1245))
- **mcptools (R)** (Categories: General-Purpose Utilities) — Posit's MIT-licensed CRAN package (1.0.1, published 2026-07-27) that turns R into an MCP server, including attachment to a live interactive session. Closes a structural gap: no Bioconductor-specific MCP server exists, yet the catalog already carries seven R/Bioconductor-based skills with no catalogued route to an R runtime ([CRAN](https://cran.r-project.org/web/packages/mcptools/index.html), [vignette](https://posit-dev.github.io/mcptools/articles/server.html))

### Verified (no changes)
- `anthropics/life-sciences` marketplace re-fetched — 21 plugins, all already catalogued, zero diff.
- MCPmed organisation swept in full (12 repos): `GEOmcp` and `allenbrain-mcp` already catalogued; `STRINGmcp` recorded as a pending augment to the existing STRING page rather than a duplicate entry; `PLSDBmcp` located but declined — no LICENSE file, the only MCPmed repo without one.
- MCP Registry `single-cell` search returned zero servers; a directed search confirmed **no CRISPR / gRNA-design MCP server or Skill exists** anywhere — recorded as a standing Molecular and Cellular Biology gap.
