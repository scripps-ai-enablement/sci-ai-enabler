...
- `domain_group` — exactly one of the seven controlled values above. `domain` stays free-text for the page's detail line; `domain_group` is what the table groups by, so pick the closest fit rather than inventing a new group. Singletons (a lone math, spatial, plant-science, or visualization system) belong in `Math & symbolic` or `Other`, not a new heading. **This is a closed vocabulary that `summary.md` renders by iterating exactly these values** — if you ever add or rename a group, update the `group_order` list in `summary.md` to match, or systems in the changed group fall through to the page's `⚠ Uncategorized` safety-net section instead of their proper heading.
...
- **Keep the synthesis qualitative and self-checking.** The synthesis describes the *shape* of the field — which domains are most loop-closed, where the strongest evidence sits, what is newest, where evaluation is heading. **Do not hardcode per-group counts in the prose** — the table already shows them and the total renders dynamically via `{{ systems | size }}`. Phrase superlatives so they survive growth, and treat any superlative ("largest", "newest frontier", "most loop-closed") as a claim that must still be true against the current grouping.
- **Re-verify the synthesis every run — this is the forward-thinking safeguard.** After creating or updating system pages, re-read the whole synthesis against the freshly grouped table and confirm every claim still holds: the group you call "largest" actually is (count the table), "newest frontier" has not been overtaken, named exemplars are still apt and still in the catalogue, and the dynamic-count sentence reads naturally. Correct anything that has drifted, then bump `synthesis_reviewed` in `summary.md`'s front-matter to today. **Re-verify on this 30-day cadence even on a run that adds no new systems** (mirror the per-system `last_verified` rule): if `synthesis_reviewed` is more than 30 days old, re-read and refresh the synthesis regardless. The synthesis is the one piece of hand-written prose left on the page, so it is the one piece that can silently go stale — this check is what keeps it honest as the field evolves.
- **Rewrite synthesis structure only when a cross-cutting pattern genuinely shifts** — a new strongest-evidence result, a genuinely new `domain_group`, or a reordering of which domains lead. A system that merely fits existing patterns needs only its system page (the table absorbs it) plus, at most, a one-line correction if it tipped a superlative. Never add a per-system paragraph.
...
- **Phase B — write and verify**: do the pure file editing. Read `.coscientist-candidates.md`, create each `autonomous-science/systems/<slug>.md` page (with complete controlled front-matter — the Landscape table auto-renders from it, so no per-system prose is added to `summary.md`) from the handoff plus the PDF's `.txt` sidecar in `sources/`, update `summary.md` synthesis or `evaluation.md` only if a cross-cutting pattern has materially shifted, re-verify any system whose `last_verified` is more than 30 days old, re-verify the synthesis against the current grouping and bump `synthesis_reviewed` whenever you added/removed a system or it is more than 30 days stale, append a dated block to `COSCIENTIST_CHANGELOG.md`, and delete the handoff file. Phase B's tool list excludes all web and MCP tools — if you find yourself wanting to fetch something new, log it under `Deferred — next-run priority` in `autonomous-science/curator-state.md` instead.
